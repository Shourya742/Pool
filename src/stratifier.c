...
struct stratum_instance
{
	...
	double diff_change_job_id; /* Last job_id we changed diff */
							 ...
							 const int msg_type)
{
	...
	LOGWARNING("Sending stratum message %s", stratum_msgs[msg_type]);
	msg = ckzalloc(sizeof(smsg_t));
	msg->json_msg = val;
...
{
	LOGWARNING("Parse subscribe %s", json_dumps(params_val, JSON_EOL | JSON_COMPACT));
...
static void read_userstats(ckpool_t *ckp, sdata_t *sdata, int tvsec_diff)
{
	...
	while ((dir = readdir(d)) != NULL)
	{
		...
		json_array_foreach(worker_array, index, arr_val)
		{
			...
			json_get_double(&lastshare, arr_val, "lastshare");
									  ...
									  const char *workername)
{
	...
	if (new_user)
	{
		LOGWARNING("Added new user %s%s", username, user->btcaddress ? " as address based registration" : "");
	}
...
static void stratum_send_diff(sdata_t *sdata, const stratum_instance_t *client)
{
	LOGWARNING("-------------------- stratum send diff -----------------------------------");
	json_t *json_msg;

	LOGWARNING("------------- client->diff -------------------- %f", client->diff);
	JSON_CPACK(json_msg, "{s[f]soss}", "params", client->diff, "id", json_null(),
			   "method", "mining.set_difficulty");

	LOGWARNING("Value in diff, %s", json_dumps(json_msg, JSON_EOL | JSON_COMPACT));
...
static void stratum_send_message(sdata_t *sdata, const stratum_instance_t *client, const char *msg)
{
	LOGWARNING("-------------------- stratum_send_message -----------------------------------");
	json_t *json_msg;
...
{
	LOGWARNING("-------------------- stratum_broadcast_update -----------------------------------");
	json_t *json_msg;
...
{
	LOGWARNING("-------------------- stratum_send_update -----------------------------------");
	ckpool_t *ckp = sdata->ckp;
	json_t *json_msg;
...
static void stratum_broadcast_updates(sdata_t *sdata, bool clean)
{
	LOGWARNING("-------------------- stratum_broadcast_updates -----------------------------------");
	stratum_instance_t *client, *tmp;
	json_t *json_msg;
...
static json_params_t *create_json_params(const int64_t client_id, const json_t *method, const json_t *params,
										 const json_t *id_val)
{
	json_params_t *jp = ckalloc(sizeof(json_params_t));
	LOGWARNING("client_id %d", client_id);
	LOGWARNING("id_val: %lld", json_integer_value(id_val));
	LOGWARNING("method_val: %s", json_string_value(method));
	LOGWARNING("params: %s", json_dumps(params, 0));
...
static void parse_method(ckpool_t *ckp, sdata_t *sdata, stratum_instance_t *client,
						 ...
						 json_t *params_val)
{
	LOGWARNING("-------------------- parse_method -----------------------------------");
	const char *method;
	LOGWARNING("id_val: %lld", json_integer_value(id_val));
	LOGWARNING("method_val: %s", json_string_value(method_val));
	LOGWARNING("params_val: %s", json_dumps(params_val, JSON_EOL | JSON_COMPACT));
	...
	if (cmdmatch(method, "mining.subscribe"))
	{
		...
		LOGWARNING("Value in subscribe, %s", json_dumps(val, JSON_EOL | JSON_COMPACT));
	...
	{
		json_params_t *jp;
		LOGWARNING("HERE>>>>> %s %s %d %d", client->identity, client->address, client->authorised, client->subscribed);
...
{
	LOGWARNING("-------------------- parse_subscribe_result -----------------------------------");
	int len;
...
static void parse_authorise_result(ckpool_t *ckp, sdata_t *sdata, stratum_instance_t *client,
								   json_t *val)
{
	LOGWARNING("-------------------- parse_authorise_result -----------------------------------");
	if (!json_is_true(val))
	{
...
{
	LOGWARNING("-------------------- generate_remote_user -----------------------------------");
	char *base_username = strdupa(workername), *username;
	sdata_t *sdata = ckp->sdata;
	bool new_user = false;
	user_instance_t *user;
	...
	if (new_user)
	{
		LOGWARNING("Added new remote user %s%s", username, user->btcaddress ? " as address based registration" : "");
	}
...
{
	LOGWARNING("-------------------- parse_remote_shareerr -----------------------------------");
...
{
	LOGWARNING("-------------------------------- parse_instance_msg----------------------------------------");
	json_t *val = msg->json_msg, *id_val, *method, *params;
	LOGWARNING("Value: %s", json_dumps(val, JSON_EOL | JSON_COMPACT));
