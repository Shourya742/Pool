...
static void send_client_json(ckpool_t *ckp, cdata_t *cdata, int64_t client_id, json_t *json_msg)
{
	...
	LOGWARNING("---------------- send_client_json----------------------");
	LOGWARNING("name %s, ", ckp->name);
	LOGWARNING("Message incoming : %s", json_dumps(json_msg, JSON_EOL | JSON_COMPACT));
